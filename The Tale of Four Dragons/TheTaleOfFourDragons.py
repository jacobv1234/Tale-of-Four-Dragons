...
def savegame():
    ...
    file.write(name + ';' + room_name + ';' + str(player.hp) + ';'+str(player.maxhp)+';'+str(player.magic)+';'+str(player.maxmagic)+';'+player.weapon+';'+str(player.exp)+';'+str(player.level)+';'+str(player.money))
    file.close()
...
if selected == 'Continue':
    ...
    for item in contents:
        addtoweapons(item)
    file.close()
    file = open('SaveData/spells.txt','r')
    contents = file.read().split(';')
    del contents[-1]
    for i in range(0,len(contents),2):
        addtospells(i,int(i+1))
    file.close()
    ...
    for i in range(len(contents)):
        flags[flagkeys[i]] = int(contents[i])
    file.close()
    room_name = othercontents.pop(0)
    player.hp = int(othercontents.pop(0))
    player.maxhp = int(othercontents.pop(0))
    player.magic = int(othercontents.pop(0))
    player.maxmagic = int(othercontents.pop(0))
    player.weapon = othercontents.pop(0)
    player.exp = int(othercontents.pop(0))
    player.level = int(othercontents.pop(0))
    player.money = int(othercontents.pop(0))
    update_stats()
    oldlevel = player.level - 1
    if flags['forestpearlgot'] == 1:
        addpearl(1)
    if flags['fieldpearlgot'] == 1:
        addpearl(2)
    if flags['mountainpearlgot'] == 1:
        addpearl(3)
    if flags['oceanpearlgot'] == 1:
        addpearl(4)
    if flags['desertpearlgot'] == 1:
        addpearl(5)
    if flags['skypearlgot'] == 1:
        addpearl(6)
    if flags['swordcouragegot'] == 1:
        addpearl(7)
    if flags['icegemgot'] == 1:
        addgem(1)
    if flags['watergemgot'] == 1:
        addgem(2)
    if flags['firegemgot'] == 1:
        addgem(3)
    if flags['stormgemgot'] == 1:
        addgem(4)
    if flags['crowngot'] == 1:
        addgem(5)

file = open('Areas/Rooms/'+ room_name +'.json')
room = json.load(file)
file.close()
